<?php
  try
{
    require_once('XoSoAPNAndroidPushConfig.php');
    ...
    $obj = new XOSO_APNS_Android_Push($config);
    $obj->start();
}
catch (Exception $e)
{
...
class XOSO_APNS_Android_Push
{
           ...
           while (true)
            {
                ...
                $stmt = $this->pdo->prepare('SELECT * FROM x_notice_queue Where os_type = 2  LIMIT 100');
                $stmt->execute();
                   ...
                   if ($this->send_push_notification($message->id,$message->device_token, $message->payload))
                    {
                        ...
                        $stmt = $this->pdo->prepare('Delete From x_notice_queue  WHERE id = ?');
   ...
   function send_push_notification($messageId,$registatoin_ids, $message) {
       try {
               ...
               $headers = array(
                    'Authorization: key=' . "AIzaSyAP0SjYjv12WjNT1jsyEd_0n-oT2OONdsc",
