...
class APNS_Android_Push
{
           ...
           while (true)
            {
                ...
                $stmt = $this->pdo->prepare('SELECT * FROM g_notice_queue Where os_type = 2 And channel =  2   LIMIT 100');
                $stmt->execute();
   ...
   function send_push_notification($messageId,$registatoin_ids, $message) {
        ...
        $headers = array(
            'Authorization: key=' . "AIzaSyAYpUP9Y0R2jnShn1TfZYm0aHoWU-goyHk",
            'Content-Type: application/json'
        );
        
        echo "AIzaSyAYpUP9Y0R2jnShn1TfZYm0aHoWU-goyHk---channel22";
