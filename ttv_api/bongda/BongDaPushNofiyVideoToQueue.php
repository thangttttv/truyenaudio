  ...
  try
{
   ...
   // writeToLog("Notify_Push_Queue script started ($mode mode)");
    ...
    $obj = new Video_Notify_Push_Queue($config);
...
{
    global $config;
    if ($fp = fopen($config['logfileVideoQueue'], 'at'))
    {
...
class Video_Notify_Push_Queue
{
                ...
                foreach ($listNews as $news)
                {
                    ...
                    while($i<1000000000){
                        ...
                        foreach($arrTokenCup as $token){
                             echo "---->Video: ".$news["id"]."-".$token["device_token"]."-".$token["os_type"]."\n";
                              $payload = $this->createPayLoadNotify($news["title"],3,$news["id"],0,$icon);
                               if(!empty($payload))
                               $this->insertNotifyToQueue(0,$news["id"],3,$token["device_token"],$payload,$news["create_date"],$token["os_type"]);
                        }
                        $i++;
                    }
                    ...
                    //Send To Token Form Club Favorite
                     ...
                     $this->updateHadNofifyVideo($news["id"]);
  ...
  function insertNotifyToQueue($notice_id,$object_id,$object_type,$device_token,$payload,$time_sent,$os_type)
    ...
    function updateHadNofifyVideo($video_id)
    {   
        try{
            $connect = $this->connectBongDaDB();
            $sql = "UPDATE fb_video SET had_notify  = 1 WHERE id  = ?";
            # creating the statement
            $q = $connect->prepare($sql);
            $arrV = array($video_id);
            $count = $q->execute($arrV);
  ...
  function getAllVideo()
    {
        $connect = $this->connectBongDaDB();
        $sql = "SELECT id,title,image,cup_id,club_id,create_date FROM fb_video WHERE STATUS = 1 AND had_notify = 0; ";
            ...
            while($row = $q->fetch()) {
                 $arr[$i] =  $row;
                 $arr[$i]['image'] = "http://kenhkiemtien.com/upload/bongda/video/".date("Y/md",strtotime($row['create_date']))."/".$row['image'];              
                 $i++;
            }
